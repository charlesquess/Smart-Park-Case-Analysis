...
在进行架构推演前，我对附件中的 3.3 万条历史告警数据进行了深度的 Pandas 探索分析。系统 L1-L4 的架构设计灵感正是来源于以下三个核心数据发现：
...
**4. PoC 实验数据印证（呼应交付物 4）**

为了验证上述"快慢双轨 + 四级降噪漏斗"在真实物理世界的可行性，我基于附件 `alerts_7days.csv` 编写了 C++ 核心状态机并进行了全量数据流的 PoC 验证（详见交付物 4 源码与运行日志）。
...
考虑到园区内存在人类与机器人的“异构运力”，且包含复杂的时空打包逻辑，我们将本场景严格建模为一个带技能约束、软硬时间窗与空间隔离的异构多车辆路径规划变体问题。
**A. 集合与参数定义**
...
面对带时间窗的动态异构车辆路径规划，寻找全局最优解是一个典型的 NP-Hard 问题。尽管 C++ 前置收敛引擎已将单日工单压降至约 1098 单，但采用传统的整数线性规划（ILP）仍面临状态空间爆炸导致的超时风险。因此，系统采用一套**“事件驱动的混合启发式算法（Event-driven Hybrid Heuristics）”**，在 $O(N)$ 的时间复杂度内实现局部最优与底线保障。调度流严格分为三个梯次：
